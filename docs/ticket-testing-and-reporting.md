...
`/spec-test` then turns the validated result into a permanent automated test on the **same surface it
...
|------------|-------|-------|
| Validate a ticket's ACs live → record results → transition → report | [`commands/test-ticket.md`](../commands/test-ticket.md) | ticket via the provider abstraction; system via the SUT plugin |
...
- **Learnings sink** — `test-ticket` can publish run learnings if a store is configured, but no
