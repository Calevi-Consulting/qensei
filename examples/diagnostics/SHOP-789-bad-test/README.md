...
It lives outside `packs/` on purpose, so the real regression gate (`make test`) stays green.

```bash
python3 -m engine.diagnose --sut sut/mock-shop --pack examples/diagnostics/SHOP-789-bad-test
```
